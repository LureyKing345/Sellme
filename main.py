...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "7908931052")
    ...
    try:
        ...
        for sess_doc in active_sessions:
            ...
            try:
                ...
                # --- FIXED SEND MESSAGE BLOCK ---
                # We do not use markdown on the link itself to avoid the parse error
                msg_text = (
                    f"🎉 **Your VIP Group is Ready!**\n\n"
                    f"🛡️ *Content protection enabled.*\n"
                    f"🔗 Link: {invite.link}\n"
                    f"⏳ Expires: {days} days.\n\n"
                    f"📥 _Historical content is uploading..._"
                )
                
                # Use MarkdownV2 or just avoid markdown on the link part to stop the crash

...
# ==========================================
# ⚡ GLOBAL HTTP SESSION (Biggest Speed Fix)
# ==========================================
_http_session: aiohttp.ClientSession = None

def get_http_session() -> aiohttp.ClientSession:
    """Returns the global shared HTTP session. Much faster than creating new ones."""
...
NUM_MIRROR_WORKERS = 3  # ⚡ 3 parallel workers for faster bulk mirroring

async def mirror_worker(worker_id: int):
    """Background worker for mirroring. Multiple workers run in parallel."""
    logger.info(f"🚀 Mirror Worker #{worker_id} started.")
    while True:
        task = await mirror_queue.get()
        try:
            admin_uid, ctype, fid, token, text, cat, wait_mid = task
            logger.info(f"🔄 Worker #{worker_id} processing: {str(fid)[:10]}...")
...
async def save_user(user_id, username, first_name):
    try:
        await users_col.update_one(
            {"user_id": user_id},
            {"$set": {"username": username, "first_name": first_name, "last_active": datetime.now().isoformat()}},
            upsert=True
        )
    except Exception as e:
...
async def api_call(method, payload=None, token=None, retries=3):
    """⚡ Uses global shared HTTP session instead of creating new one each time."""
    if token is None:
        token = BOT_TOKEN
    ...
    for attempt in range(retries):
        try:
            async with session.post(url, json=payload or {}, timeout=aiohttp.ClientTimeout(total=45)) as resp:
                res = await resp.json()
                if resp.status == 429:
                ...
                if resp.status != 200:
                    if method == 'editMessageText' and "there is no text in the message to edit" in res.get('description', ''):
                        ...
                        logger.warning(f"TG API '{method}' failed. Status: {resp.status} - {res}")
                return res
        except Exception as e:
            logger.error(f"Network error on '{method}' attempt {attempt + 1}: {e}")
            await asyncio.sleep(2)
    return {}

async def download_tg_file(token, file_id):
    """⚡ Uses global shared HTTP session."""
...
# ==========================================
def _get_file_ext(ctype: str) -> str:
    """Returns correct file extension for each content type."""
    return {"video": "mp4", "photo": "jpg", "document": "bin"}.get(ctype, "bin")

async def _upload_to_backup_bot(b_token, admin_uid, cat, ctype, file_bytes, filename=""):
    """⚡ Uses global session. Fixed file extension for all types."""
    ext = _get_file_ext(ctype)
    upload_filename = filename if filename else f"mirror.{ext}"

    data = aiohttp.FormData()
    data.add_field("chat_id", str(admin_uid))
    data.add_field("caption", "🔄 Storage")
    data.add_field("disable_notification", "true")
    data.add_field(ctype, file_bytes, filename=upload_filename)

    try:
        session = get_http_session()
        async with session.post(
            f"https://api.telegram.org/bot{b_token}/send{ctype.capitalize()}",
            data=data
        ) as resp:
            res = await resp.json()
            if res.get("ok"):
                msg_id = res["result"]["message_id"]
                result_data = res["result"][ctype]
                fid = result_data[-1]["file_id"] if isinstance(result_data, list) else result_data["file_id"]
                # Delete the storage message silently
                await session.post(
                    f"https://api.telegram.org/bot{b_token}/deleteMessage",
                    json={"chat_id": admin_uid, "message_id": msg_id}
                )
                return {"token": b_token, "file_id": fid}
    except Exception as e:
        logger.error(f"Mirror to ...{b_token[-6:]} failed: {e}")
    return None
...
async def mirror_content_background(admin_uid, ctype, primary_file_id, primary_token, text_content, cat, wait_mid):
    ...
    if not backup_bots or ctype == "text":
        if wait_mid:
            await api_call("editMessageText", {
                "chat_id": admin_uid, "message_id": wait_mid,
                "text": "✅ **Saved!** (No backup bots configured).", "parse_mode": "Markdown"
            }, token=primary_token)
        return
    ...
    if file_bytes:
        tasks = [
            _upload_to_backup_bot(bot["token"], admin_uid, cat, ctype, file_bytes)
            for bot in backup_bots if bot.get("token")
        ]
        results = await asyncio.gather(*tasks)
        new_mirrors = [r for r in results if r]

        if new_mirrors and existing_doc:
            await content_col.update_one(
                {"_id": existing_doc["_id"]},
                {"$push": {"mirrors": {"$each": new_mirrors}}}
            )

    if wait_mid:
        await api_call("editMessageText", {
            "chat_id": admin_uid, "message_id": wait_mid,
            "text": "✅ **Saved & Mirrored!**", "parse_mode": "Markdown"
        }, token=primary_token)
...
async def send_content_to_group(group_id, content_doc):
    ...
    if not ctoken:
        logger.error("No ACTIVE Uploader Bot configured! Skipping content.")
        return
    ...
    if target_fid:
        ...
        if ctype == "photo":
            payload["photo"] = target_fid
        elif ctype == "video":
            payload["video"] = target_fid
        elif ctype == "document":
            payload["document"] = target_fid
        await api_call(f"send{ctype.capitalize()}", payload, token=ctoken)
    else:
        ...
        if file_bytes:
            ...
            data.add_field(ctype, file_bytes, filename=f"healed.{ext}")
            try:
                session = get_http_session()
                async with session.post(
                    f"https://api.telegram.org/bot{ctoken}/send{ctype.capitalize()}", data=data
                ) as resp:
                    res = await resp.json()
                    if res.get("ok"):
                        result_data = res["result"][ctype]
                        new_fid = result_data[-1]["file_id"] if isinstance(result_data, list) else result_data["file_id"]
                        await content_col.update_one(
                            {"_id": content_doc["_id"]},
                            {"$push": {"mirrors": {"token": ctoken, "file_id": new_fid}}}
                        )
            except Exception as e:
...
async def generate_vip_group(user_id, cat_key, days):
    ...
    try:
        await api_call("sendMessage", {
            "chat_id": user_id,
            "text": "✅ **Payment Confirmed!**\n\n⚙️ _Generating your private VIP group..._",
            "parse_mode": "Markdown"
        }, token=BOT_TOKEN)
        ...
        if not active_sessions:
            ...
            await api_call("sendMessage", {
                "chat_id": user_id,
                "text": "❌ System error: No group creators available. Admin needs to upload sessions."
            }, token=BOT_TOKEN)
            return
        ...
        for sess_doc in active_sessions:
            ...
            logger.info(f"Attempting group generation with Session: ...{sess_id[-6:]}")
            ...
            try:
                ...
                if not await client.is_user_authorized():
                    logger.warning(f"Session ...{sess_id[-6:]} unauthorized. Marking inactive.")
                ...
                result = await client(functions.channels.CreateChannelRequest(
                    title=f"VIP: {cat_name} - {user_id}", about="Exclusive access.", megagroup=True
                ))
                ...
                await client(functions.channels.EditAdminRequest(
                    channel=created_chat, user_id='me',
                    admin_rights=ChatAdminRights(
                        anonymous=True, change_info=True, post_messages=True, edit_messages=True,
                        delete_messages=True, ban_users=True, invite_users=True, pin_messages=True,
                        add_admins=True, manage_call=True
                    ), rank="Owner"
                ))
                ...
                if main_bot_username:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to invite main bot: {e}")
                ...
                if uploader_username:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to invite uploader: {e}")
                ...
                await subs_col.insert_one({
                    "user_id": user_id, "group_id": bot_api_group_id, "category": cat_key,
                    "expiry_at": expiry, "session_used": sess_id[-6:], "last_synced_at": now_str
                })
                await api_call("sendMessage", {
                    "chat_id": user_id,
                    "text": f"🎉 **Your VIP Group is Ready!**\n\n🛡️ *Content protection enabled.*\n🔗 Link: {invite.link}\n⏳ Expires: {days} days.\n\n📥 _Historical content is uploading..._",
                    "parse_mode": "Markdown",
                    "reply_markup": {"inline_keyboard": [[{"text": "Join VIP Group", "url": invite.link}]]}
                }, token=BOT_TOKEN)
                ...
                for c in contents:
                    await send_content_to_group(bot_api_group_id, c)
                ...
                if "flood" in err_msg or "too many requests" in err_msg:
                    logger.warning(f"Session ...{sess_id[-6:]} rate-limited. Skipping.")
                elif any(k in err_msg for k in ["frozen", "banned", "deactivated", "expired", "invalid", "unregistered"]):
                    logger.warning(f"Session ...{sess_id[-6:]} is dead. Marking inactive.")
...
async def create_crypto_invoice(user_id, cat_key, days, network):
    ...
    payload = {
        "amount": price, "pay_currency": "USDT", "network": network,
        "order_id": order_id, "callback_url": f"{WEBHOOK_URL}/oxapay_callback"
    }
    session = get_http_session()
    async with session.post(
        "https://api.oxapay.com/v1/payment/white-label",
        json=payload, headers={"merchant_api_key": OXAPAY_KEY}
    ) as resp:
        if resp.status == 200:
            data = (await resp.json()).get("data")
            await payments_col.insert_one({
                "user_id": user_id, "order_id": order_id,
                "category": cat_key, "days": int(days), "status": "pending"
            })
...
async def create_star_invoice_link(uid, cat, days, ptoken):
    ...
    payload = {
        "title": f"VIP Access - {cat_data['name']}", "description": f"{days} Days Access",
        "payload": f"sub_{cat}_{days}_{uid}", "currency": "XTR",
        "prices": [{"label": "VIP Access", "amount": price}]
    }
...
def build_help_text(is_admin: bool) -> str:
    user_cmds = (
        "📋 **Available Commands:**\n\n"
        "👤 **User Commands:**\n"
        "/start — Browse categories & buy VIP\n"
        "/help — Show this help message\n"
    )
    admin_cmds = (
        "\n🔧 **Admin Commands (Main Bot):**\n"
        "/sessions — Enter session upload mode\n"
        "/sesdn — Close session upload mode\n"
        "/help — Show all commands\n"
        "\n📦 **Admin Commands (Content Bot):**\n"
        "/bulkmodestart `<cat_key>` — Start bulk upload\n"
        "/bulkmodeend — Stop bulk upload mode\n"
        "\n⚙️ **Admin Panel:**\n"
        "Visit your Render admin URL with `?pass=YOUR_PASSWORD`\n"
        "\n📊 **Routes:**\n"
        "`/health` — Check bot health\n"
        "`/webhook` — Main bot webhook\n"
        "`/payment_webhook` — Payment bot webhook\n"
        "`/content_webhook/{token}` — Content bot webhook\n"
        "`/oxapay_callback` — Crypto payment callback\n"
    )
    return user_cmds + (admin_cmds if is_admin else "")
...
async def send_duration_menu(uid, cat_key, categories, mid=None):
    try:
        ...
        kb = {
            "inline_keyboard": [
                [{"text": f"{v['label']} - ${v.get('crypto_price', 0)}", "callback_data": f"days_{cat_key}_{k}"}]
                for k, v in cat_data['plans'].items()
            ]
        }
        ...
        if cat_data.get("image") and cat_data["image"].strip():
            ...
            await api_call("sendPhoto", {
                "chat_id": uid, "photo": cat_data["image"].strip(),
                "caption": caption, "parse_mode": "Markdown", "reply_markup": kb
            }, token=BOT_TOKEN)
        else:
            if mid:
                res = await api_call("editMessageText", {
        ...
        logger.error(f"send_duration_menu error: {e}")
...
async def process_update(update):
    try:
        ...
        if "message" in update:
            ...
            # --- SESSION CONVERTER (Admin Only) ---
            if uid in admin_ids:
                state_doc = await admin_states.find_one({"uid": uid})

                if text == "/sessions":
                    await admin_states.update_one(
                        {"uid": uid},
                        {"$set": {"state": "uploading_sessions"}}, upsert=True
                    )
                    await api_call("sendMessage", {
                        "chat_id": uid,
                        "text": "📂 **Session Converter Mode:**\n\nSend `.session` files here. I will extract String Session, save it to DB, and delete the file.\n\nSend `/sesdn` when done.",
                        "parse_mode": "Markdown"
                    }, token=BOT_TOKEN)
                    return
                ...
                if text == "/sesdn":
                    ...
                    await api_call("sendMessage", {
                        "chat_id": uid,
                        "text": f"✅ Session mode closed. **{count}** active string sessions in DB.",
                        "parse_mode": "Markdown"
                    }, token=BOT_TOKEN)
                    return
                ...
                if state_doc and state_doc.get("state") == "uploading_sessions":
                    if "document" in msg and msg["document"]["file_name"].endswith(".session"):
                        ...
                        await api_call("sendMessage", {
                            "chat_id": uid,
                            "text": f"⏳ Downloading and converting `{fname}`...",
                            "parse_mode": "Markdown"
                        }, token=BOT_TOKEN)
                            ...
                            try:
                                ...
                                if await client.is_user_authorized():
                                    ...
                                    if string_session:
                                        ...
                                        await api_call("sendMessage", {
                                            "chat_id": uid,
                                            "text": f"✅ Converted `{fname}` → String Session saved to DB!",
                                            "parse_mode": "Markdown"
                                        }, token=BOT_TOKEN)
                                    else:
                                        await api_call("sendMessage", {
                                            "chat_id": uid,
                                            "text": f"❌ Error: Extracted string was empty. Corrupted session?"
                                        }, token=BOT_TOKEN)
                                else:
                                    await api_call("sendMessage", {
                                        "chat_id": uid,
                                        "text": f"❌ Session `{fname}` is not authorized."
                                    }, token=BOT_TOKEN)
                            except Exception as e:
                                logger.error(f"Session conversion error: {e}")
                                await api_call("sendMessage", {
                                    "chat_id": uid,
                                    "text": f"❌ Error converting `{fname}`: {str(e)}"
                                }, token=BOT_TOKEN)
                            finally:
                                try:
                                    await client.disconnect()
            ...
            if text.startswith("/start"):
                ...
                # ✅ NEW: Help command showing all commands
                admin_ids_list = await get_admin_ids()
                is_admin = uid in admin_ids_list
                help_text = build_help_text(is_admin)
                await api_call("sendMessage", {
                    "chat_id": uid,
                    "text": help_text,
                    "parse_mode": "Markdown"
            ...
            if data == "back_main":
                ...
                res = await api_call("editMessageText", {
                    "chat_id": uid, "message_id": mid,
                    "text": "📚 **Select Category:**", "parse_mode": "Markdown", "reply_markup": kb
                }, token=BOT_TOKEN)
                if not res or not res.get("ok"):
                    await api_call("deleteMessage", {"chat_id": uid, "message_id": mid}, token=BOT_TOKEN)
                    await api_call("sendMessage", {
                        "chat_id": uid, "text": "📚 **Select Category:**",
                        "parse_mode": "Markdown", "reply_markup": kb
                    }, token=BOT_TOKEN)

            elif data.startswith("cat_"):
                # ✅ FIX: Use split with maxsplit to handle underscores in cat_key
                cat = data.split("_", 1)[1]
                await send_duration_menu(uid, cat, categories, mid)

            elif data.startswith("days_"):
                # ✅ FIX: rsplit to handle underscores in cat_key
                # Format: days_{cat}_{days_number}
                without_prefix = data[5:]  # remove "days_"
                d = without_prefix.rsplit("_", 1)[1]
                cat = without_prefix.rsplit("_", 1)[0]

                plan = categories[cat]['plans'][d]
                kb_buttons = []
                if plan.get('star_price', 0) > 0:
                    kb_buttons.append([{"text": "⭐️ Telegram Stars", "callback_data": f"pay_{cat}_{d}_stars"}])
                if plan.get('crypto_price', 0) > 0:
                    kb_buttons.append([{"text": "💎 USDT Crypto", "callback_data": f"pay_{cat}_{d}_crypto"}])
                kb_buttons.append([{"text": "🔙 Back", "callback_data": f"cat_{cat}"}])

                res = await api_call("editMessageText", {
                    "chat_id": uid, "message_id": mid,
                    "text": "💳 **Select Payment Method:**", "parse_mode": "Markdown",
                    "reply_markup": {"inline_keyboard": kb_buttons}
                }, token=BOT_TOKEN)
                if not res or not res.get("ok"):
                    await api_call("deleteMessage", {"chat_id": uid, "message_id": mid}, token=BOT_TOKEN)
                    await api_call("sendMessage", {
                        "chat_id": uid, "text": "💳 **Select Payment Method:**",
                        "parse_mode": "Markdown", "reply_markup": {"inline_keyboard": kb_buttons}
                    }, token=BOT_TOKEN)

            elif data.startswith("pay_"):
                # ✅ FIX: rsplit for method (stars/crypto), then split remaining
                # Format: pay_{cat}_{d}_{method}
                method = data.rsplit("_", 1)[1]
                without_prefix = data[4:]  # remove "pay_"
                without_method = without_prefix.rsplit("_", 1)[0]  # remove method
                d = without_method.rsplit("_", 1)[1]
                cat = without_method.rsplit("_", 1)[0]
                ...
                if method == "stars":
                    ...
                    if not ptoken:
                        await api_call("sendMessage", {
                            "chat_id": uid,
                            "text": "❌ Admin has not configured the payment bot."
                        }, token=BOT_TOKEN)
                        return

                    res = await api_call("editMessageText", {
                        "chat_id": uid, "message_id": mid,
                        "text": "⏳ Generating Secure Invoice..."
                    }, token=BOT_TOKEN)
                    if not res or not res.get("ok"):
                        await api_call("deleteMessage", {"chat_id": uid, "message_id": mid}, token=BOT_TOKEN)
                        sent_msg = await api_call("sendMessage", {
                            "chat_id": uid, "text": "⏳ Generating Secure Invoice..."
                        }, token=BOT_TOKEN)
                    ...
                    if invoice_url:
                        kb = {"inline_keyboard": [
                            [{"text": "⭐️ Pay Now", "url": invoice_url}],
                            [{"text": "🔙 Back", "callback_data": f"days_{cat}_{d}"}]
                        ]}
                        res = await api_call("editMessageText", {
                            "chat_id": uid, "message_id": mid,
                            "text": "⭐️ **Secure Checkout**\n\nClick below to securely purchase your access.",
                            "parse_mode": "Markdown", "reply_markup": kb
                        }, token=BOT_TOKEN)
                        if not res or not res.get("ok"):
                            await api_call("deleteMessage", {"chat_id": uid, "message_id": mid}, token=BOT_TOKEN)
                            await api_call("sendMessage", {
                                "chat_id": uid, "text": "⭐️ **Secure Checkout**",
                                "parse_mode": "Markdown", "reply_markup": kb
                            }, token=BOT_TOKEN)
                        await payments_col.update_one(
                            {"user_id": uid, "category": cat, "status": "pending"},
                            {"$set": {"message_id": mid, "days": int(d)}}, upsert=True
                        )

                elif method == "crypto":
                    kb = {"inline_keyboard": [
                        [{"text": "USDT (BSC)", "callback_data": f"net_{cat}_{d}_BSC"}],
                        [{"text": "USDT (TRX)", "callback_data": f"net_{cat}_{d}_Tron"}],
                        [{"text": "🔙 Back", "callback_data": f"days_{cat}_{d}"}]
                    ]}
                    res = await api_call("editMessageText", {
                        "chat_id": uid, "message_id": mid,
                        "text": "🔗 **Select Network:**", "parse_mode": "Markdown", "reply_markup": kb
                    }, token=BOT_TOKEN)
                    if not res or not res.get("ok"):
                        await api_call("deleteMessage", {"chat_id": uid, "message_id": mid}, token=BOT_TOKEN)
                        await api_call("sendMessage", {
                            "chat_id": uid, "text": "🔗 **Select Network:**",
                            "parse_mode": "Markdown", "reply_markup": kb
                        }, token=BOT_TOKEN)

            elif data.startswith("net_"):
                # ✅ FIX: rsplit for network, then for days
                # Format: net_{cat}_{d}_{network}
                net = data.rsplit("_", 1)[1]
                without_prefix = data[4:]  # remove "net_"
                without_net = without_prefix.rsplit("_", 1)[0]
                d = without_net.rsplit("_", 1)[1]
                cat = without_net.rsplit("_", 1)[0]
                ...
                if invoice:
                    res = await api_call("sendPhoto", {
                        "chat_id": uid,
                        "photo": invoice['qr_code'],
                        "caption": f"<b>Deposit {invoice['pay_amount']} USDT</b>\nNetwork: {net}\nAddress: <code>{invoice['address']}</code>",
                        "parse_mode": "html"
                    }, token=BOT_TOKEN)
                    if res and res.get("ok"):
                        await payments_col.update_one(
                            {"order_id": invoice['merchant_order_id']},
                            {"$set": {"message_id": res["result"]["message_id"]}}
                        )

                if wait_msg.get("result"):
                    await api_call("deleteMessage", {
                        "chat_id": uid, "message_id": wait_msg["result"]["message_id"]
                    }, token=BOT_TOKEN)
...
async def process_payment_update(update):
    try:
        ...
        if "pre_checkout_query" in update:
            await api_call("answerPreCheckoutQuery", {
        ...
        if "message" in update and "successful_payment" in update["message"]:
            ...
            pending = await payments_col.find_one_and_update(
                {"user_id": target_uid, "category": cat, "status": "pending"},
                {"$set": {"status": "completed", "order_id": order_id}}
            )
            if pending and "message_id" in pending:
                await api_call("deleteMessage", {
                    "chat_id": target_uid, "message_id": pending["message_id"]
                }, token=BOT_TOKEN)
            await api_call("deleteMessage", {
    ...
    try:
        if "message" in update:
            ...
            if text.startswith("/bulkmodestart "):
                ...
                await bulk_sessions_col.update_one(
                    {"admin_id": uid},
                    {"$set": {"category": cat, "active": True}}, upsert=True
                )
                await api_call("sendMessage", {
                    "chat_id": uid,
                    "text": f"🟢 **Bulk Mode Started: `{cat}`**\n\nForward media now. Saves instantly, mirrors in background.",
                    "parse_mode": "Markdown"
                }, token=ctoken)
                return
            ...
            if active_session and not text.startswith("/"):
                ...
                # 🛠 QUEUE FOR BACKGROUND MIRRORING
                if fid:
...
async def handle_health(request):
    ...
    return web.json_response({
        "status": "ok",
        "mirror_queue_size": mirror_queue.qsize(),
...
async def handle_oxapay_webhook(request):
    try:
        ...
        async with session.post(
            ...
            json={"merchant": OXAPAY_KEY, "trackId": str(track_id)}
        ) as resp:
            if (await resp.json()).get("status", "").lower() in ["paid", "success", "200"]:
                ...
                if payment:
                    await payments_col.update_one(
                        {"_id": payment["_id"]}, {"$set": {"status": "completed"}}
                    )
                    if "message_id" in payment:
                        await api_call("deleteMessage", {
                            "chat_id": payment["user_id"], "message_id": payment["message_id"]
                        }, token=BOT_TOKEN)
...
async def start_background_tasks(app):
    ...
    # ⚡ Start global HTTP session
    get_http_session()
    logger.info("🌐 Global HTTP session initialized.")

    # ⚡ Start 3 parallel mirror workers
    for i in range(1, NUM_MIRROR_WORKERS + 1):
        asyncio.create_task(mirror_worker(i))
    logger.info(f"🚀 {NUM_MIRROR_WORKERS} Mirror Workers started.")
...
if __name__ == '__main__':
    ...
    # Routes
    app.router.add_get('/health', handle_health)                          # ✅ NEW: Health check
    ...
    app.on_cleanup.append(close_http_session)                             # ✅ NEW: Clean shutdown

    logger.info(f"🚀 Bot online. Webhooks on port {PORT}. Use /help to see all commands.")
